...
func Test_GenerateApiKey(t *testing.T) {
	...
	t.Run("Successfully generates API key", func(t *testing.T) {
		desc := "test-1"
		rawKey, apiKey, err := service.GenerateApiKey(portaineree.User{ID: 1}, desc)
		is.NoError(err)
		is.NotEmpty(rawKey)
		is.NotEmpty(apiKey)
	...
	t.Run("Api key prefix is 7 chars", func(t *testing.T) {
		rawKey, apiKey, err := service.GenerateApiKey(portaineree.User{ID: 1}, "test-2")
		is.NoError(err)
	...
	t.Run("Api key has 'ptr_' as prefix", func(t *testing.T) {
		rawKey, _, err := service.GenerateApiKey(portaineree.User{ID: 1}, "test-x")
		is.NoError(err)
	...
	t.Run("Decoded raw api-key digest matches generated digest", func(t *testing.T) {
		rawKey, apiKey, err := service.GenerateApiKey(portaineree.User{ID: 1}, "test-4")
		is.NoError(err)
...
func Test_UpdateAPIKey(t *testing.T) {
	...
	t.Run("Successfully updates api-key in cache upon api-key update", func(t *testing.T) {
		_, apiKey, err := service.GenerateApiKey(portaineree.User{ID: 1}, "test-x2")
		is.NoError(err)
...
func Test_InvalidateUserKeyCache(t *testing.T) {
	...
	t.Run("User key eviction does not affect other users keys", func(t *testing.T) {
		...
		user1 := portaineree.User{ID: 1}
		_, apiKey1, err := service.GenerateApiKey(user1, "test-1")
		is.NoError(err)

		user2 := portaineree.User{ID: 2}
