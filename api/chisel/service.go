...
func (service *Service) checkTunnels() {
	...
	for key, tunnel := range service.tunnelDetailsMap {
		...
		if tunnel.Status == portaineree.EdgeAgentManagementRequired && time.Since(tunnel.LastActivity) < requiredTimeout {
			continue
		}

		if tunnel.Status == portaineree.EdgeAgentActive && time.Since(tunnel.LastActivity) < activeTimeout {
			continue
		}
	...
	for endpointID, tunnel := range tunnels {
		...
		if tunnel.Status == portaineree.EdgeAgentManagementRequired && elapsed > requiredTimeout {
			log.Debug().
				Int("endpoint_id", int(endpointID)).
				Str("status", tunnel.Status).
				Float64("status_time_seconds", elapsed.Seconds()).
		...
		if tunnel.Status == portaineree.EdgeAgentActive && elapsed > activeTimeout {
			log.Debug().
				Int("endpoint_id", int(endpointID)).
				Str("status", tunnel.Status).
				Float64("status_time_seconds", elapsed.Seconds()).
