...
func (client *ContainerServicesClient) ListKubernetesVersions(ctx context.Context, location string) (res []string, err error) {
	...
	for _, o := range *l.Orchestrators {
		if o.OrchestratorType != nil && *o.OrchestratorType == string(cs.Kubernetes) {
...
func (client *VirtualMachineSizesClient) ListMachineTypes(ctx context.Context, location string) (res MachineTypes, err error) {
...
// ListRegions returns all regions available at the specified subscription
func (client *RegionClient) ListRegions(ctx context.Context) (res []string, err error) {
	regions, err := client.ListByService(ctx, "azure-test", "apis")
	if err != nil {
		return nil, err
	}
