...
func (e *EksProvisioner) GetCluster(name string) (*Cluster, error) {
	...
	if err != nil {
		return nil, clouderrors.NewErrFatal("could not create temp file for kubeconfig, err: %v", err)
	}
	...
	if err != nil {
		return nil, clouderrors.NewErrFatal("read kubeconfig failed, err: %v", err)
	}

	var config Config
	err = yaml.Unmarshal(b, &config)
	if err != nil {
		return nil, clouderrors.NewErrFatal("unmarshal kubeconfig failed, err: %v", err)
	}
	...
	if err != nil {
		return nil, clouderrors.NewErrFatal("marshal kubeconfig failed, err: %v", err)
	}
