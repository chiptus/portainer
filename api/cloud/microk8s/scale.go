...
type (
	...
	Microk8sScalingRequestFactory struct {
		...
		setMessage     func(summary, detail string, operatingStatus portaineree.EndpointOperationStatus) error
...
func (s *Microk8sScalingRequestFactory) Process() error {
	...
	s.setMessage("Scaling cluster", "Scaling in progress", portaineree.EndpointOperationStatusProcessing)

	endpoint, err := s.dataStore.Endpoint().Endpoint(s.scalingRequest.EndpointID)
	if err != nil {
		details := fmt.Sprintf("Scaling error: %v", err)
		s.setMessage("Failed to scale cluster", details, portaineree.EndpointOperationStatusWarning)
	...
	if len(s.scalingRequest.MasterNodesToAdd) > 0 || len(s.scalingRequest.WorkerNodesToAdd) > 0 {
		s.setMessage("Scaling cluster", "Scaling up in progress", portaineree.EndpointOperationStatusProcessing)
		err = mk8sCluster.AddNodes(endpoint, credentials, s.scalingRequest)
	} else if len(s.scalingRequest.NodesToRemove) > 0 {
		s.setMessage("Scaling cluster", "Scaling down in progress", portaineree.EndpointOperationStatusProcessing)
	...
	s.setMessage("Scaling up cluster", "Scaling complete", portaineree.EndpointOperationStatusDone)
	return nil
}
