...
func Test_customTemplateGitFetch(t *testing.T) {
	...
	// create user(s)
	user1 := &portaineree.User{ID: 1, Username: "user-1", Role: portaineree.StandardUserRole, PortainerAuthorizations: authorization.DefaultPortainerAuthorizations()}
	err := store.User().Create(user1)
	is.NoError(err, "error creating user 1")

	user2 := &portaineree.User{ID: 2, Username: "user-2", Role: portaineree.StandardUserRole, PortainerAuthorizations: authorization.DefaultPortainerAuthorizations()}
