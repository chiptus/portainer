...
func (handler *Handler) endpointEdgeJobsLogs(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	if featureflags.IsEnabled(portaineree.FeatureNoTx) {
		err = handler.getEdgeJobLogs(handler.DataStore, endpoint.ID, portaineree.EdgeJobID(edgeJobID), payload)
	} else {
...
func (handler *Handler) getEdgeJobLogs(tx dataservices.DataStoreTx, endpointID portaineree.EndpointID, edgeJobID portaineree.EdgeJobID, payload logsPayload) error {
	...
	err = handler.FileService.StoreEdgeJobTaskLogFileFromBytes(strconv.Itoa(int(edgeJobID)), strconv.Itoa(int(endpoint.ID)), []byte(payload.FileContent))
	if err != nil {
