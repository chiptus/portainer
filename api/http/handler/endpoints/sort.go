...
func sortEnvironmentsByField(environments []portaineree.Endpoint, environmentGroups []portainer.EndpointGroup, sortField sortKey, isSortDesc bool) {
	if sortField == "" {
		return
	}

	var less comp[portaineree.Endpoint]
	switch sortField {
	case sortKeyName:
		less = func(a, b portaineree.Endpoint) int {
	...
	slices.SortStableFunc(environments, func(a, b portaineree.Endpoint) int {
		mul := 1
		if isSortDesc {
			mul = -1
		}
