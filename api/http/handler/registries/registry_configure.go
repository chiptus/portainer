...
func (handler *Handler) deleteProxy(registry *portaineree.Registry) {
	id := strconv.Itoa(int(registry.ID))
	handler.registryProxyService.DeleteProxy(id)
	if registry.Type == portaineree.GitlabRegistry {
		handler.registryProxyService.DeleteProxy(id + "-gitlab")
	}
}
