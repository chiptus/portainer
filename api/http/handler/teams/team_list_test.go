...
func Test_teamList(t *testing.T) {
	...
	h := NewHandler(requestBouncer, testhelpers.NewUserActivityService())
	h.DataStore = store
	...
	// create users
	userWithEndpointAccessByTeam := &portaineree.User{ID: 2, Username: "standard-user-inherit-endpoint-access-from-team", Role: portaineree.StandardUserRole, PortainerAuthorizations: authorization.DefaultPortainerAuthorizations()}
	...
	endpointWithTeamAccessPolicy := &portaineree.Endpoint{ID: 1, GroupID: endpointGroupOnly.ID, TeamAccessPolicies: teamAccessPolicies}
	...
	// create endpoint
	endpointUnderGroupWithTeam := &portaineree.Endpoint{ID: 2, GroupID: endpointGroupWithTeam.ID}
