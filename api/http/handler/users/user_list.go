...
func (handler *Handler) userList(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	canAccessEndpoint := make([]portaineree.User, 0)
...
func sanitizeUser(user *portaineree.User) {
	user.Password = ""
	user.EndpointAuthorizations = nil
	user.ThemeSettings = portaineree.UserThemeSettings{}
	user.OpenAIApiKey = ""
...
func sanitizeUsers(users []portaineree.User) {
	for i := range users {
