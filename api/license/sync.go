...
func (service *Service) Metadata(licenses []liblicense.PortainerLicense) (liblicense.Metadata, error) {
	...
	// Prefill the map with 0 counts. This prevents a bug where the license
	// server will never set an endpoint's count back to 0 when you remove the
	// last endpoint of that type.
	for i := 1; i <= 18; i++ {
		endpointMap[instanceID+":"+strconv.Itoa(i)] = 0
	}
	for i, e := range endpoints {
		err = snapshot.FillSnapshotData(service.dataStore, &endpoints[i])
		if err != nil {
			log.Err(err).Msg("failed reading snapshot data for endpoint")
			continue
		}
		...
		if endpointutils.IsEdgeEndpoint(&e) && !e.UserTrusted {
			// Don't count waiting room endpoints.
			continue
		}
...
func (service *Service) authMethod() int {
	...
	case portainer.AuthenticationOAuth:
		switch {
		case strings.HasPrefix(settings.OAuthSettings.AuthorizationURI, "https://login.microsoftonline.com/a/oauth2/v2.0/authorize"):
			return int(MetadataAuthOauthMicrosoft)
		case settings.OAuthSettings.AuthorizationURI == "https://accounts.google.com/o/oauth2/auth":
			return int(MetadataAuthOauthGoogle)
		case settings.OAuthSettings.AuthorizationURI == "https://github.com/login/oauth/authorize":
	...
	for _, e := range endpoints {
		...
		case portaineree.EdgeAgentOnDockerEnvironment, portaineree.EdgeAgentOnKubernetesEnvironment:
			...
			if snapshot.Docker != nil {
				count += snapshot.Docker.TotalCPU
			}
			if snapshot.Kubernetes != nil {
				count += int(snapshot.Kubernetes.TotalCPU)
			}
		}
	}
...
const (
	// If adding a new type here, increase the 0 prefill loop in the Metadata
	// function.
