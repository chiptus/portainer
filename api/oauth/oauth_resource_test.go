...
func Test_getTeams(t *testing.T) {
	...
	t.Run("returns empty list for non-existent oauth claim name", func(t *testing.T) {
		oauthSettings := &portaineree.OAuthSettings{
		...
		teams, err := getTeams(datamap, oauthSettings)
		assert.NoError(t, err, "getTeams should return no error when OAuthClaimName value not found in oauth resource")
		assert.Empty(t, teams, "getTeams should return empty list when OAuthClaimName value not found in oauth resource")
	})
