...
func (service *PendingActionsService) executePendingAction(pendingAction portainer.PendingActions, endpoint *portaineree.Endpoint) error {
	...
	case "UpsertPortainerK8sClusterRoles":
		...
		if err != nil {
			return fmt.Errorf("failed to get kube client for endpoint %d: %w", endpoint.ID, err)
		}
		...
		if err != nil {
			...
			return fmt.Errorf("failed to upsert portainer kubernetes cluster roles for endpoint %d: %w", endpoint.ID, err)
		}
		return nil
	}
	return nil
