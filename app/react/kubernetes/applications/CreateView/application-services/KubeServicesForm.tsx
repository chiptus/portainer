...
export function KubeServicesForm({
        ...
        {services.map((service, index) => (
          <div key={index} className="border-bottom py-6">
            {service.Type ===
              KubernetesApplicationPublishingTypes.CLUSTER_IP && (
              <>
                <div className="text-muted vertical-center">
                  <Icon icon={List} />
                  ClusterIP
                </div>
                <div className="row">
                  <div className="col-sm-9 service-annotations !p-0">
                    <Annotations
                      index={index}
                      initialAnnotations={services[index].Annotations}
                      handleUpdateAnnotations={handleUpdateAnnotations}
                      errors={annotationsErrors[index]}
                    />
                  </div>
                </div>
                <ClusterIpForm
  ...
  // annotations are currently handled outside of formik and yup.
  function handleUpdateAnnotations(annotations: Annotation[], index?: number) {
    if (index === undefined) return;
    const newServices = [...services];
    newServices[index].Annotations = annotations;
    onChange(newServices);
