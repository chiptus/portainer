...
export function AppIngressPathsForm({
  ...
  const { data: restrictAccessToIngress, ...restrictUserIngressQuery } =
    useEnvironment(
      environmentId,
      (environment) =>
  ...
  if (restrictUserIngressQuery.isError) {
    return <FormError>Unable to check if ingress use is restricted.</FormError>;
  }
  ...
  if (!isAdmin && restrictAccessToIngress) {
    return null;
  }
