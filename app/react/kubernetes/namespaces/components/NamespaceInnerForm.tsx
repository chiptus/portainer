...
import { LoadBalancerQuotaFormValues } from './LoadBalancerFormSection/types';
import { StorageQuotaFormValues } from './StorageQuotaFormSection/types';
...
export function NamespaceInnerForm({
  ...
  return (
    <Form>
      ...
      <Annotations
        placeholder={['e.g. app.kubernetes.io/name', 'examplename']}
        initialAnnotations={initialValues.annotations}
        errors={errors.annotations}
        handleUpdateAnnotations={(annotations) =>
          setFieldValue('annotations', annotations)
        }
      />
      ...
      />
      {useLoadBalancer && (
        <LoadBalancerFormSection
          values={values.loadBalancerQuota}
          onChange={(loadBalancerQuota: LoadBalancerQuotaFormValues) =>
            setFieldValue('loadBalancerQuota', loadBalancerQuota)
          }
          errors={errors.loadBalancerQuota}
        />
      )}
      ...
      {storageClasses.length > 0 && (
        <StorageQuotaFormSection
          storageClasses={storageClasses}
          values={values.storageQuota}
